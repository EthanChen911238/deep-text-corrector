...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers=1):
        ...
        # 將input size個詞變成 hidden size 維度的向量表示
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, last_context, last_hidden, encoder_outputs):
        ...
        # attn_tmp = attn_weights.unsqueeze(1)
        # attn_tmp2 = attn_tmp.bmm(encoder_outputs)
        # context = attn_tmp2.squeeze(1)
...
class Attn(nn.Module):
    ...
    def score(self, hidden, encoder_output):
        ...
        if self.method == 'dot':
            ...
            # tmp = hidden.unsqueeze(1)
            # tmp2 = energy.unsqueeze(2)
            # energy = tmp.bmm(tmp2)
