...
def save_state(encoder, decoder, encoder_optim, decoder_optim, step, path='D:\GEC\deep-text-corrector-master\checkpoints\model'):
    state = {'step': step,
             'encoder': encoder.state_dict(),
             'decoder': decoder.state_dict(),
             'encoder_optim': encoder_optim.state_dict(),
             'decoder_optim': decoder_optim.state_dict()}
    filename = path + '\model-' + str(step)+'.pt'
    torch.save(state, filename)


def load_state(step=None, path='D:\GEC\deep-text-corrector-master\checkpoints\model'):
    state = {}
    # 查找路徑
