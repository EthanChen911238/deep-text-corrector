...
def sequence_mask(sequence_length, max_len=None):
    ...
    # a.expand_as(b) 將a擴充到跟b一樣的維度 -> a : 2*3 ; b : 2*6 after c : 2*6
...
def train(input_batch, len_inputs, target_batch, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion):
    ...
    print("batch_size = ", batch_size, "target_length = ",target_length)

    # TODO parameter를 paddingsequence로 받게끔 하고 아래는 삭제
    # 將target_batch中每個tensor的長度取出來
    length_targets = Variable(torch.LongTensor(list(map(lambda s: len(s), target_batch)))).cuda()
...
for step in range(step, final_steps + 1):
    ...
    # inputs : embedding後的輸入句子
    # len_inputs : 輸入句子的長度
    # target : embedding後的期望輸出句子
    # len_target : 期望輸出句子的長度
    inputs, targets, len_inputs, len_targets = train_corpus.next_batch(5)
